...
class UserCommands(commands.Cog):
    ...
    @app_commands.command(name="등록", description="복구봇 라이센스 코드를 사용해 이 서버의 이용 기간을 등록/연장합니다.")
    ...
    @app_commands.command(name="역할", description="유저가 웹 페이지에서 인증을 성공적으로 마치면 자동으로 지급할 역할을 지정합니다.")
    @app_commands.checks.has_permissions(administrator=True)
    ...
    # --- [수정됨] 수동 명령어에서도 서버 ID 입력 추가 ---
    @app_commands.command(name="복구키사용", description="[수동 방식] 구매하신 1회성 복구키를 수동으로 입력하여 사용합니다.")
    @app_commands.checks.has_permissions(administrator=True)
    async def use_recovery_key(self, interaction: discord.Interaction, 데이터를_가져올_서버_id: str, 복구키: str):
        from main import KeyModal
        modal = KeyModal()
        modal.server_id_input.default = 데이터를_가져올_서버_id
